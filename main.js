...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==============================================================
    // FIXED: HARD RESET TO 1 WHEN SWITCHING TO GUEST
    // ==============================================================
    window.updateBookingType = function(index, type) {
        ...
        if (type === 'couple') {
            freshItems[index].adults = 2; // Lock to 2 for couple
        } else {
    ...
    window.renderCart = function() {
        ...
        window.bookingItems.forEach((item, index) => {
            ...
            if (isTransferItem) {
                ...
                // Discount logic: Go and Return is a 1.9 multiplier instead of 2. ($50 becomes $95)
                    ...
                    <div class="border-t border-gray-200 pt-4 flex flex-col md:flex-row gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <select onchange="window.updateItemTripType(${index}, this.value)" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F27D57] outline-none cursor-pointer bg-white">
                                ...
                                <option value="return" ${tripType === 'return' ? 'selected' : ''}>Go & Return (Discounted)</option>
                            </select>
                        </div>
                        <div class="flex-1">
                            ...
                            <select onchange="window.updateItemVehicle(${index}, this.value)" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F27D57] outline-none cursor-pointer bg-white text-sm">
                                ...
                                <option value="1 - 12 Pax = Mini Bus" ${vehicle === '1 - 12 Pax = Mini Bus' ? 'selected' : ''}>1 - 12 Pax = Mini Bus (+40%)</option>
                                <option value="1 - 24 Pax = Bus" ${vehicle === '1 - 24 Pax = Bus' ? 'selected' : ''}>1 - 24 Pax = Bus (+96%)</option>
                            </select>
                        </div>
                    </div>
                `;
